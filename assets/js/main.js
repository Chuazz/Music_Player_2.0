...
const myApp = {
    ...
    loadCurrentSong: function () {
        ...
        myVar.songName.innerHTML = `${currentSong.name}`;
        myVar.songAuthor.innerHTML = `${currentSong.singer}`;
        myVar.headerImg.querySelector("img").src = `${currentSong.image}`;
    ...
    playRandom: function () {
        ...
        myVar.songedId.push(this.currentIndex);

        this.currentIndex = random;
        while(myVar.songedId.includes(random) && myVar.songedId.length < songList.length){
            random = Math.floor(Math.random() * songList.length);
        }
        this.currentIndex = random;
        
        if(myVar.songedId.length > songList.length){
            random = Math.floor(Math.random() * songList.length);
            myVar.songedId = [random];
        }
        this.scrollToSong();
    },

    // #error
    // Đang chạy mà đổi sang tab khác là đứng nhạc
    // Chuyển sang tab khác
    changeTabHanlde: function (e) {
        var currentSongList = songList;
        var updateSong = e.target.closest(".update");
        ...
        if(liked){
            ...
            else if(likedSong.length < 2){
                myVar.$("body").style = "height: 100vh";
            }

            else{
                this.currentIndex = 0;
                myVar.$("body").style = "height: auto";
        ...
        if(liked || mostlyPlay){
            if(this.isPlaying === true){
                alert("Nhạc đang chơi mà chuyển sang danh sách khác là nó đứng á🤦‍♀️load lại trang i");
            }
    ...
    handleEvents: function () {
        var imgSize = myVar.headerImg.scrollWidth;
        var imgAnimate = myVar.headerImg.animate([
        ...
        window.addEventListener("scroll", function() {
            ...
            myVar.headerImg.style.width = newSize + "px";
            myVar.headerImg.style.height = newSize + "px";
            myVar.headerImg.style.opacity = `${opacity}`;
        });
        ...
        myVar.audio.addEventListener("play", () => {
            myVar.header.classList.add("playing");
            this.isPlaying = true;
            imgAnimate.play();  
        });
        ...
        myVar.playlist.addEventListener("click", (e) => {
            var song = e.target.closest(".song__content-body:not(.active)");
            var downloadBtn = e.target.closest(".fa-download");
            var likeSong = e.target.closest(".fa-heart");
            var parent = myFunc.getParentElement(e.target, ".song__content-body");
            
            if( song && !downloadBtn && !likeSong){
                myVar.$(".song__content-body").classList.remove(".active");
        ...
        window.addEventListener("keydown", (e) => {
            switch (e.code) {
                ...
                case "ArrowDown": {
                    ...
                    // console.log( myVar.songVolumeRange.value)
                    // this.volumeLogic();
                    e.preventDefault();
                    break;
                }
                ...
                default:{
                    console.log("Chưa biết nút này làm gì luôn😂");
                    console.log(e.code) 
                    break;
                }
            }
